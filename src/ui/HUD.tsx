...
const EVIDENCE_PARTICLES = Array.from({ length: 16 }, (_, index) => ({
  angle: index * 22.5 + (index % 3) * 5,
    ...
    <div className="pointer-events-none absolute inset-0 select-none">
      ...
      {snap.evidenceDiscovery && (
        <div
          ...
          className="evidence-reveal absolute left-1/2 top-5 z-[60] w-[24rem] max-w-[calc(100vw-2rem)] -translate-x-1/2"
        >
          <div className="evidence-reveal-burst" aria-hidden="true">
            <i className="evidence-reveal-ring" />
            <i className="evidence-reveal-ring evidence-reveal-ring--late" />
            {EVIDENCE_PARTICLES.map((particle, index) => (
              <i
                key={index}
                className="evidence-reveal-particle"
                style={{
          ...
          <div className="evidence-reveal-card overflow-hidden rounded border border-[#8ab8a0]/70 bg-[#0b1110]/95 shadow-[0_0_36px_rgba(76,132,108,0.3)] backdrop-blur-md">
            <div className="h-px bg-gradient-to-r from-transparent via-[#8ab8a0] to-transparent" />
            <div className="flex items-center gap-3 px-4 py-3">
              <div className="evidence-reveal-sprite flex h-16 w-16 shrink-0 items-center justify-center rounded border border-[#49685b] bg-[#111816] shadow-inner">
              <img
              ...
              <div className="min-w-0">
                <div className="text-[9px] uppercase tracking-[0.24em] text-[#8ab8a0]">{snap.evidenceDiscovery.kind === 'physical' ? 'Evidence discovered' : 'Association revealed'}</div>
