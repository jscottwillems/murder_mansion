...
export function DialoguePanel({ game, snap }: { game: Game; snap: Snapshot }) {
  ...
  return (
    <div className="absolute inset-x-0 bottom-0 flex justify-center pb-6">
      <div className="w-[42rem] max-w-[92vw] rounded-t-lg border border-[#3a352a] border-b-0 bg-[#0d0c12]/95 p-5 shadow-2xl backdrop-blur">
        <div className="flex items-stretch gap-4">
          <GuestPortrait archetypeId={guest.archetypeId} name={guest.name} alive={guest.alive} size="dialog" />
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-3 min-h-16 rounded border border-[#2a2822] bg-black/40 p-3">
              {iv.lastQuestion && (
                <div className="mb-1 text-xs italic text-[#7a8ab0]">You: {iv.lastQuestion}</div>
              )}
